...
class Eliminator(object):
    ...
    def remove_edges(self, others):
        """Remove edges that depend on relation between others and self.region.
            ...
            while not finished:
                ...
                for source, target in self.map_g.edges_iter():
                    ...
                    if len(bad) == 2:
                        ...
                        self.map_g.remove_edge(source, target)
                        self.map_g.remove_edge(target, source)
                        break
