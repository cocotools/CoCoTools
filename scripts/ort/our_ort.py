...
Step 4: Add edges if necessary to make mapping graph reciprocal.

Step 5: Make connectivity queries for all regions (sans map specification)
        in the final mapping graph, and combine results in a single graph.

Step 6: Remove from the connectivity graph nodes that are not in the mapping
        graph (i.e., for which we have no mapping information).

Step 7: Perform ORT using the final mapping and connectivity graphs and PHT00
        as the target map.
"""
...
if __name__ == '__main__':
    ...
    try:
        with open('map_g4.pck') as f:
            map_g = pickle.load(f)

    except IOError:
        map_g = make_symmetrical(map_g)
        pickle_it('map_g4.pck', map_g)
    ...
    try:
        ...
        ort = Ort(map_g, conn_g, 'PHT00')
        pickle_it('final_g.pck', ort.target_g)
    ...
    try:
        ...
        final_g = remove_b(final_g)
        pickle_it('final_g2.pck', final_g)
