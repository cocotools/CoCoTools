...
def run_ort(target_map):
    ...
    if response == 'y':
        endg = coco.EndGraph()
        endg.add_translated_edges(mapg, cong, target_map)
        with open('results/post_sfn_revisions/endg%s.pck' % target_map,
                  'w') as f:
            pickle.dump(endg, f)
    elif response == 'n':
        with open('results/post_sfn_revisions/endg%s.pck' % target_map) as f:
            endg = pickle.load(f)
